    ...
    <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        <div>
            <input type="text" form="data_form" name="title"
                class="text-2xl font-semibold text-neutral-200 border-none focus:outline-none"
                value="{{ $content->title ?? 'Untitled Content' }}">
...
<div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
    <div class="bg-neutral-900 rounded-lg border border-neutral-800 p-8 flex flex-col">
        <form id="data_form" action="{{ route('contents.update', $content) }}" method="POST">
            @csrf
            @method('PUT')
            <textarea name="json_data" form="data_form" id="json_data"
                class="w-full text-neutral-300 leading-9 focus:outline-none" rows="20"
                autofocus>{!! json_encode($content->json_data,JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) !!}</textarea>
        </form>
    </div>
</div>
@endsection